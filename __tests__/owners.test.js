...
test('that * works', async () => {
  ...
  let res = await owners(createMockPR(), createMockContext(codeowner, commitDiffs), config())
  expect(res.length).toBe(1)
  expect(res[0]).toBe('bob')

  commitDiffs = createCommitDiffs(['another/file/path/test.js'])

  res = await owners(createMockPR(), createMockContext(codeowner, commitDiffs), config())
  expect(res.length).toBe(1)
  expect(res[0]).toBe('bob')
})
