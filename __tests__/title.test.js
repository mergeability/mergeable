...
test('fail gracefully if invalid setting is provided', async () => {
  ...
  let titleValidation = await title(createMockPR('Valid Title'), null, config.settings.mergeable.pull_requests)
  ...
  let config = new Configuration(`
    mergeable:
      pull_requests:
        title: '@#$@#$@#$'
  `)
  let titleValidation = await title(createMockPR('WIP Title'), null, config.settings.mergeable.pull_requests)
  expect(titleValidation.mergeable).toBe(true)
})
...
test('checks that it fail when exclude regex is in title', async () => {
  ...
  titleValidation = await title(createMockPR('(feat) WIP Title'), null, config.settings.mergeable.pull_requests)
...
test('checks that advance setting of must_include works', async () => {
  ...
  let titleValidation = await title(createMockPR('include Title'), null, config.settings.mergeable.pull_requests)
  expect(titleValidation.mergeable).toBe(false)
...
test('checks that it fail when begins_with is not in title', async () => {
  ...
  titleValidation = await title(createMockPR('(test) WIP Title'), null, config.settings.mergeable.pull_requests)
...
test('checks that it fail when ends_with is not in title', async () => {
  ...
  titleValidation = await title(createMockPR('WIP Title (test)'), null, config.settings.mergeable.pull_requests)
...
test('isMergeable is false if regex found or true if not', async () => {
  ...
  titleValidation = await title(createMockPR('Some Title'), null, config.settings.mergeable.pull_requests)
  expect(titleValidation.mergeable).toBe(true)
})
...
test('description is correct', async () => {
  ...
  titleValidation = await title(createMockPR('Just Title'), null, config.settings.mergeable.pull_requests)
