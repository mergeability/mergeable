...
class Validator extends EventAware {
  ...
  async processValidate (context, validationSettings, registry) {
    ...
    if (validationSettings.if && validationSettings.if != '') {
      let jexlContext = {
        payload: context.probotContext.payload,
      }

      if (true === jexl.evalSync(validationSettings.if, jexlContext)) {
        return Promise.resolve({
          status: 'skip',
          name: validationSettings.do,
          validations: [
            {
              status: 'skip',
              description: 'Validator not run due expression (' + validationSettings.if + ')'
            }
          ],
        })
      }
