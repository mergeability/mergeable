...
class Validator extends EventAware {
  ...
  validateSettings (supportedSettings, settingToCheck, nestings = []) {
    ...
    for (let key of Object.keys(settingToCheck)) {
      ...
      const optionType = typeof settingToCheck[key]
      if (optionType === 'object') {
        if (_.isArray(settingToCheck[key])) {
          if (supportedSettings[key] === 'array') continue
          ...
          nestings.push(key)
          this.validateSettings(supportedSettings[key], settingToCheck[key], nestings)
        }
