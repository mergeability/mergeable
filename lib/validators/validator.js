...
const octokitCache = new NodeCache({ stdTTL: 600, maxKeys: 5000 })
...
class Validator extends EventAware {
  ...
  async processValidate (context, validationSettings, registry) {
    ...
    let result = await this.validate(context, validationSettings, registry)

    result.if = {
      expression: validationSettings.if,
      skipped: false
    }

    try {
      let skipped = await this.shouldSkip(context, validationSettings.if)
      if (skipped) {
        result.status = 'skip'
        result.if = {
          expression: validationSettings.if,
  ...
  async shouldSkip (context, expr) {
    if (typeof expr === 'undefined' || expr === '') {
      return false
    }

    let octokitJexl = new OctokitJexl(context)

    let jexlContext = {
      payload: context.probotContext.payload
    }

    jexl.addFunction('getRepositoryTopics', async () => octokitJexl.getRepositoryTopics())

    return await jexl.eval(expr, jexlContext) ? false : true
  }
  ...
  validateSettings (supportedSettings, settingToCheck, nestings = []) {
    ...
    for (let key of Object.keys(settingToCheck)) {
      if (key === 'do' || key === 'if' || key === 'and' || key === 'or') continue
...
class OctokitJexl {
  constructor (context) {
    ...
    this.cache = octokitCache
  }

  async getRepositoryTopics () {
    if (process.env.USE_CONFIG_CACHE) {
      let names = this.cache.get(`${this.context.payload.full_name}#getRepositoryTopics`)
      if (names) {
        return names
      }
    }

    return this.context.octokit.repos.getAllTopics({
      owner: this.context.payload.repository.owner.login,
      repo: this.context.payload.repository.name,
    }).then(response => {
      if (process.env.USE_CONFIG_CACHE) {
        this.cache.set(`${this.context.payload.full_name}#getRepositoryTopics`, response.data.names)
      }
