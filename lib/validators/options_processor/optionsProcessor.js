...
const optionsProcessor = (validatorContext, input, rules, returnRawOutput) => {
  const output = []
  ...
  rules.forEach(rule => {
    Object.keys(rule).forEach(key => {
      ...
      if (SUPPORTED_OPTIONS.indexOf(key) === -1) {
        ...
        output.push(require(`./options/${key}`)(validatorContext, input, rule))
      }
    })
  })
...
console.log(optionsProcessor('label', ['A', 'C'], rules))
