...
const andProcessor = require('./and')
...
const orProcessor = (validatorContext, input, rule) => {
  ...
  const validated = filters.map(filter => {
    ...
    return validationProcessor(validatorContext, input, filter, true)
  })

  let isMergeable
  let DEFAULT_SUCCESS_MESSAGE = `All the requisite validations passed for 'or' option`
  let descriptions = ''
    ...
    result.forEach(validated => {
      if (validated.status === 'fail') {
        if (descriptions.length > 2) {
          descriptions += ` ${` ***AND*** `} ${validated.description}`
        } else {
