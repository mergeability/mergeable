...
const andOrProcessor = async (validatorContext, input, rule, key) => {
  ...
  const validated = map(filters, async filter => {
    if (filter.and) {
      return await andOrProcessor(validatorContext, input, filter, 'and')
    }
    if (filter.or) {
      return await andOrProcessor(validatorContext, input, filter, 'or')
    }

    // we are only passing in one item at a time, so this will only return one element array
    data = await options.process(validatorContext, input, filter, true)
