...
class Checks extends Action {
  ...
  async afterValidate (context, settings, name, results) {
    ...
    let checkStatus = settings.state

    if (results.errorCount === 0 && results.failCount > 0) {
      let allFailsIsApprovals = true
      results.failures.forEach(fail => {
        if (fail.name !== 'approvals') {
          allFailsIsApprovals = false
        }
      })
      if (allFailsIsApprovals) {
