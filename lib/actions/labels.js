...
const deleteLabels = async (context, issueNumber, labels) => {
  const currentLabels = await context.octokit.issues.listLabelsOnIssue(
  ...
  const labelsToKeep = currentLabels.data.map(label => label.name).filter(label => !matchesPatterns(label, labels))
