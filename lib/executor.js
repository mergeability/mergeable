  ...
  config.forEach(rule => {
    Promise.all(getValiationPromises(context, registry, rule))
      .then((result) => {
        ...
        Promise.all(getActionPromises(context, registry, rule, statuses))
      })
  })
}
...
const registerValidatorsAndActions = (config, registry) => {
  config.forEach(rule => {
    ...
    rule.failure.concat(rule.pass).forEach(action => {
      let key = action.do
...
const getActionPromises = (context, registry, rule, statuses) => {
  const isErrored = statuses.includes('error')
  ...
  actions.forEach(action => {
    ...
    if (actionClass.isEventSupported(eventName)) {
      promises.push(actionClass.doPostAction(context, action))
    } // else add error for event not supported
  })
