...
class V1Config {
  static transform (config) {
    ...
    if (config.issues) {
      output.push(processValidators('issues.*', config.issues, {pass: consts.DEFAULT_ISSUES_PASS, fail: consts.DEFAULT_ISSUES_FAIL, error: consts.DEFAULT_ISSUES_ERROR}))
    }
...
const processValidators = (event, config, options) => {
  ...
  return { when: event, validate, pass: options.pass, fail: options.fail, error: options.error }
}
