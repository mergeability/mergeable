...
const configCache = process.env.ENABLE_CONFIG_CACHE ? new NodeCache({ stdTTL: 0, maxKeys: 5000 }) : null
...
class Configuration {
  ...
  static async updateCache (context) {
    ...
    if (repo.repo === '.github') {
      configCache.keys().filter(key => key.startsWith(`${repo.owner}/`)).map(key => configCache.del(key))
    } else {
      configCache.del(`${repo.owner}/${repo.repo}`)
    }
  }
  ...
  static async fetchConfigFile (context) {
    ...
    const configPath = process.env.CONFIG_PATH ? process.env.CONFIG_PATH : 'mergeable.yml'
    if (configCache !== null) {
      Configuration.updateCache(context)
      config = configCache.get(`${repo.owner}/${repo.repo}`)
      if (config === undefined) {
        config = await context.probotContext.config(configPath)
        configCache.set(`${repo.owner}/${repo.repo}`, config)
      }
    } else {
      config = await context.probotContext.config(configPath)
    }
