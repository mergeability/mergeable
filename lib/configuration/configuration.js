...
class Configuration {
  ...
  static async fetchConfigFile (context) {
    ...
    let orgConfig = null
    let useOrgAsDefaultConfig = false
    if (process.env.USE_ORG_AS_DEFAULT_CONFIG !== undefined && process.env.USE_ORG_AS_DEFAULT_CONFIG === 'true') {
      useOrgAsDefaultConfig = true
    }

    if (useOrgAsDefaultConfig) {
      orgConfig = await octokit.repos.getContent({
        owner: repo.owner,
        repo: '.github',
