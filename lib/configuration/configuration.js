...
class Configuration {
  ...
  static async fetchConfigFile (context) {
    ...
    if (!_.isUndefined(process.env.USE_CONFIG_FROM_PULL_REQUEST)) {
      usePullRequestConfig = process.env.USE_CONFIG_FROM_PULL_REQUEST
    }
    if (usePullRequestConfig && ['pull_request', 'pull_request_review'].includes(context.event)) {
    // if (['pull_request', 'pull_request_review'].includes(context.event) && !process.env.USE_CONFIG_FROM_MAIN_BRANCH) {
