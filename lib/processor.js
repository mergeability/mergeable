...
const processFilters = (validatorContext, input, filters) => {
  ...
  let definedInput = ''
  if (!!input) {  // eslint-disable-line
    definedInput = input
  }

  // for backward compatibility, check if the filters is a string
  if (typeof filters === 'string') {
    return mustExclude(validatorContext, definedInput, filters)
  }
  ...
  for (let key in filters) {
    switch (key) {
      case 'must_include':
        output.push(mustInclude(validatorContext, definedInput, filters[key]))
        break
      case 'must_exclude':
        output.push(mustExclude(validatorContext, definedInput, filters[key]))
        break
      case 'min':
        output.push(min(validatorContext, definedInput, filters[key]))
        break
      case 'max':
        output.push(max(validatorContext, definedInput, filters[key]))
        break
      case 'begins_with':
        output.push(beginsWith(validatorContext, definedInput, filters[key]))
        break
      case 'ends_with':
        output.push(endsWith(validatorContext, definedInput, filters[key]))
        break
      case 'no_empty':
        output.push(noEmpty(validatorContext, definedInput, filters[key]))
        break
      case 'required':
        output.push(required(validatorContext, definedInput, filters[key]))
        break
      case 'and':
        output.push(andOr(validatorContext, definedInput, filters))
        break
      case 'or':
        output.push(andOr(validatorContext, definedInput, filters))
        break
