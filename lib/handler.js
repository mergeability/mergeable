...
class Handler {
  ...
  static async handleIssueOpened (context) {
    ...
    let includes = [ 'title', 'label', 'milestone', 'description' ]
      .filter(validator => excludes.indexOf(validator) === -1)

    includes.forEach(validator => {
      validators.push(require(`../lib/${validator}`)(issue, context, config.settings))
    })
  ...
  static async handle (context, pullRequest) {
    ...
    let settings = config.settings.mergeable.pull_requests
    let validators = []
