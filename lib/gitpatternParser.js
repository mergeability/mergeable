...
const parseOwners = (content) => {
  ...
  return {
    for: (path) => {
      let res = []
      owners.filter(([globs]) => {
        return !globs || globs.split(' ').find(glob => checkIfPathMatches(path, glob))
      }).forEach(owners => {
        res = owners.slice(1)
      })
      return res
...
const checkIfPathMatches = (path, toMatch) => {
...
const getFileName = (path) => {
  // check if path is a glob, if not, it must be a filename already
  if (!path.includes('/')) {
    return path
  }
...
module.exports = {
  parseOwners: parseOwners,
  checkIfPathMatches: checkIfPathMatches
}
