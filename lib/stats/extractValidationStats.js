 ...
 * skipCount Num of validations skipped
...
module.exports = (results) => {
  ...
  const skipCount = validationStatuses.filter(status => status === 'skip').length
  ...
  const output = {
    ...
    failures: searchByStatus('fail', results)
  }

  return output
}

function searchByStatus (nameKey, myArray) {
  let statusFiltered = []
  for (var i = 0; i < myArray.length; i++) {
    if (myArray[i].status === nameKey) {
      statusFiltered.push(myArray[i])
    }
  }
