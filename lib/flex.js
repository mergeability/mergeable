...
const processWorkflow = async (context, registry, config) => {
  ...
  await Promise.all(config.settings.map(rule => {
    if (isEventInContext(rule.when, context)) {
      return Promise.all(getValiatorPromises(context, registry, rule))
        .catch((err) => {
          let log = context.log.child({ name: 'mergeable' })
        ...
        .catch((err) => {
          console.error('An error occured while executing an action', err)
        })
    }
  }))
}
...
const getActionPromises = (context, registry, rule, result) => {
  ...
  if (actions) {
    const afterValidateFuncCall = (actionClass, context, action, result) => {
      console.log(action, result)
      return actionClass.afterValidate(context, action, result)
    }
